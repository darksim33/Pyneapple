from src.fit.model import Model


def test_mono_exp_model():
    mono_model = "( exp(-kron(b_values, abs(d1))) * (1 - (sum([]))) ) * S0"
    mono_function = Model.multi_exp_printer(1, ["d1", "S0"])

    if mono_function != mono_model:
        assert False

    assert True


def test_bi_exp_model():
    bi_model = "( exp(-kron(b_values, abs(d1))) * f1 + exp(-kron(b_values, abs(d2))) * (1 - (sum(['f1']))) ) * S0"
    bi_function = Model.multi_exp_printer(2, ["d1", "d2", "f1", "S0"])

    if bi_function != bi_model:
        assert False

    assert True


def test_tri_exp_model():
    tri_model = (
        "( exp(-kron(b_values, abs(d1))) * f1 + exp(-kron(b_values, abs(d2))) * f2 + exp(-kron(b_values, "
        "abs(d3))) * (1 - (sum(['f1', 'f2']))) ) * S0"
    )
    tri_function = Model.multi_exp_printer(3, ["d1", "d2", "d3", "f1", "f2", "S0"])

    if tri_function != tri_model:
        assert False

    assert True
