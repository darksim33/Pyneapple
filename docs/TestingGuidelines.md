...
Conventions for writing tests in Pyneapple. See also [StyleGuide.md](StyleGuide.md) for general coding conventions.
...
- Fallback to `test_<module>.py` when no clear layer applies (e.g., `test_hdf5.py`)
- Shared fixtures in `tests/conftest.py`, file/IO fixtures in `tests/_files.py`
- Import order: stdlib → third-party (`pytest`, `numpy`) → local (`from pyneapple.models import ...`)
...
class TestMonoExpModel:
    """Test suite for Monoexponential model functionality."""

    def test_model_initialization(self, monoexp_model_params):
        """MonoExp model initializes correctly with valid parameters."""
        model = MonoExpModel(*monoexp_model_params)
...
- Classes: `Test<Class>` or `Test<Feature>` — e.g., `TestCurvefitSolverFitting`
- Fixtures: lowercase with type prefix — `ivim_params`, `nnls_result`, `biexp_fitdata`
