...
@pytest.mark.gpu
def test_gpu_fit_mono_red(gpufit):
...
def test_gpu_fit_biexp(gpufit):
    ...
    # Generate known signal
    signal = S0_tissue_true * np.exp(-b_values * D_true) + S0_perfusion_true * np.exp(
        -b_values * Dp_true
    )
    fit_data = signal.reshape(1, -1).astype(np.float32)
    ...
    assert np.allclose(
        result[0][0], [S0_tissue_true, D_true, S0_perfusion_true, Dp_true], rtol=0.01
    )


@pytest.mark.gpu
def test_gpu_fit_biexp_red(gpufit):
...
@pytest.mark.gpu
def test_gpu_fit_biexp_s0(gpufit):
...
@pytest.mark.gpu
def test_gpu_fit_triexp(gpufit):
    # Known signal: S = S0_tissue * exp(-b*D) + S0_perfusion * exp(-b*Dp) + S0_vascular * exp(-b*Dv)
    b_values = np.array([0, 50, 100, 200, 400, 600, 800, 1000], dtype=np.float32)

    # Ground truth parameters
    S0_tissue_true, D_true, S0_perfusion_true, Dp_true, S0_vascular_true, Dv_true = (
        100.0,
        0.001,
        50.0,
        0.02,
...
@pytest.mark.gpu
def test_gpu_fit_triexp_red(gpufit):
...
@pytest.mark.gpu
def test_gpu_fit_triexp_s0(gpufit):
    ...
    starts = [0.5, 0.001, 0.25, 0.02, 0.1, 210]
    ...
    assert np.allclose(
        result[0][0], [f1_true, D_true, f2_true, Dp_true, Dv_true, S0_true], rtol=0.01
    )


@pytest.mark.gpu
def test_gpu_fit_mono_t1(gpufit):
    ...
    starts = [0.0015, 180, 700]
    lower = [0.0007, 10, 500]
    upper = [0.003, 2500, 3000]
    start_values = np.tile(np.float32(starts), (1, 1))
    constraints = np.tile(
        np.float32(list(zip(lower, upper))).flatten(),
        (1, 1),
    ...
    assert np.allclose(result[0][0], [D_true, S0_true, T1_true], rtol=0.01)
