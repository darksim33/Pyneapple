"""Tests for base parameter class functionality.

This module tests the BaseParams class which provides core parameter handling
functionality for all fitting approaches:

- B-value loading: Reading diffusion weighting parameters from .bval files
- Pixel arguments: Preparing per-pixel data for fitting
- Segment arguments: Computing mean signals over ROIs/segments
- Boundary management: Interfacing with boundary dictionaries
- Data validation: Ensuring proper array shapes and value ranges

Key test scenarios:
- Single and multiple segment handling with random noise
- Edge cases: single voxel segments, nonexistent segments
- Mean calculation accuracy across different signal patterns
- Boundary value extraction for IVIM models
"""
...
class TestBaseParameters:
    ...
    def test_load_b_values(self, root):
        """Test that b-values are correctly loaded from .bval file."""
    ...
    def test_get_seg_args_seg_number(self, img, seg, seg_number):
        """Test that get_seg_args returns single argument tuple for each segment number."""
        parameters = BaseParams()
        args = parameters.get_seg_args(img, seg, seg_number)
        assert len(list(args)) == 1


class TestSegmentMeanCalculation:
    """Test suite for segment-wise mean signal calculation."""

    @pytest.mark.parametrize(
        "seed", [42, 123, 456]
    )  # Multiple random seeds for reproducibility
    def test_get_seg_args_mean(self, seed):
    ...
    def test_get_seg_args_mean_multiple_segments(self):
        """Test that mean calculation works correctly with multiple different segments having distinct signals."""
        np.random.seed(789)
    ...
    def test_get_seg_args_mean_edge_cases(self):
        """Test edge cases for segment mean calculation including single voxel and nonexistent segments."""
...
class TestBoundaryIntegration:
    """Test suite for boundary dictionary integration with parameters."""

    def test_boundaries_ivim(self, ivim_bi_params):
        """Test that boundary values are correctly extracted from IVIMBoundaryDict for bi-exponential model."""
